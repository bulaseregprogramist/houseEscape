...
class Draw:
    ...
    @staticmethod
    def draw_mainmenu(
        screen: pygame.surface.Surface, author_name, boosty_name,
        da_name, textures: tuple[pygame.surface.Surface]) -> None:
        """
        Отрисовка кнопочек главного меню
        
        Args:
            screen (pygame.surface.Surface): Переменная экрана
            author_name (): Имя автора (текст)
            boosty_name (): Текст для кнопки Boosty
            da_name (): Текст для кнопки DonationAlerts
            textures (tuple[pygame.surface.Surface]): Текстуры кнопок
        """
        ...
        screen.blit(textures[8], (460, 680))
        screen.blit(textures[10], (180, 660))
        
        screen.blit(author_name, (200, 250))
        screen.blit(da_name, (10, 120))
        screen.blit(boosty_name, (650, 120))
    ...
    def show_interfaces(config) -> None:
        ...
        of = OtherFunctional(
            config.n, config.screen, config.he_map, 
        ...
        if key == 1:  # Верстак
            CraftingTable(config.screen, text, config.n, 
                        config.screen, config.player)
        elif key == 2:  # Лампа
            of.lamp(config.use)
    ...
    def render_location(
        ...
        if index == [0, 2]:  # Окрестности дома
            ...
            enemy = Enemy(self.__x3, self.__y3, "blinder", logger,
                        screen, player)
            ...
            enemy = Enemy(self.__x, self.__y, "watcher", logger,
                        screen, player)
