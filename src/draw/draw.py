...
class Draw:
    ...
    @staticmethod
    def draw_mainmenu(
        screen: pygame.surface.Surface, textures: tuple[pygame.surface.Surface]
    ) -> None:
    ...
    def show_interfaces(config) -> None:
        ...
        of = OtherFunctional(
            config.n, config.screen, config.he_map, config.player, config.logger
        )
        key = int(config.i)
        save = Saving()
        ...
        if key == 1:  # Верстак
            CraftingTable(config.screen, text, config.n, config.screen, config.player)
        elif key == 2:  # Лампа
            of.lamp(config.use)
    ...
    def render_location(
        ...
        if index == [0, 2]:  # Окрестности дома
            ...
            enemy = Enemy(self.__x2, self.__y2, "stalker", logger, screen, player)
            self.__x2, self.__y2 = enemy.enemy_draw_and_move(player, mp, num)
        elif index == [1, 2]:
            screen.blit(self.__bg3, (0, 0))
            enemy = Enemy(self.__x3, self.__y3, "blinder", logger, screen, player)
            ...
            enemy = Enemy(self.__x, self.__y, "watcher", logger, screen, player)
